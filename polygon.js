...
export class Polygon {
  ...
  animate(ctx, moveX) {
    ...
    // 방향의 문제! 마우스 클릭 후 오른쪽으로 드래그(moveX가 +가 됨.)하면 반시계방향(각도를 변화량을 -로 전달해야 함.)으로 돌리고 싶고
    // 왼쪽으로 드래그(moveX가 -가 됨)하면 시계방향(각도의 변화량을 +로 전달해야 함)으로 돌리고 싶어서 -=로 부호를 바꿔서 더해준 것.
    // 2. 또 하나! 왜 * 0.008을 하나? 
    // app.js에서 프레임마다 moveX * 0.92 하게 했으니 그냥 -= moveX 이렇게 하더라도 감속해서 언젠가 멈추긴 하겠지만,
    // moveX를 쌩으로 넣어버리면 너무 빨리 감속해버림. 즉, 너무 빨리 돌았다가 너무 빨리 멈춰버림. 각도가 너무 빨리 변한다는 거!
    ...
    for (let i = 0; i < this.sides; i++) {
      ...
      ctx.translate(x, y); // for문에서 만든 각각의 꼭지점 좌표를 원점으로(0, 0)으로 설정해 각각의 사각형의 중심으로 하고
      // 참고로 여기서 translate을 하게 된거는 translate(this.x + x, this.y + y); 로 원점을 옮겨준 거임
      ctx.rotate(((360 / this.sides) * i + 45) * Math.PI / 180);
      // 얘는 변의 갯수에 맞춰 생성된 '사각형들에 각자의 기울기를 줘서' 예쁘게 꽃모양으로 정렬될 수 있도록 함.
      // 그니까 지금 Math.PI / 180는 라디안으로 변환할 때 곱하는 값이니까 상관없고
      // ((360 / this.sides) * i + 45) 얘가 중요한데, 변의 개수대로(예를 들어 지금은 15개) 360도를 나누면 24지?
      // 그래서 사각형마다 45도, 69도, 93도... 이런식으로 기울기를 주는 거임.
