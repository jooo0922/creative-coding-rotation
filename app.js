...
class App {
  ...
  animate() {
    ...
    // 그런데 항상 똑같은 moveX값이 들어가면 한번 돌렸을 때 계속 돌아감. 감속이 안됨. 속도 변화가 없게 됨.
  ...
  onMove(e) {
    if (this.isDown) {
      ...
      // 결과적으로 moveX에는 2가지 정보가 들어가게 됨.
      // 1. pointer를 얼마나 많이(빨리) 움직였는가(= moveX의 절댓값)
      // 2. pointer를 어느 방향으로 움직였는가(= moveX 값의 부호)
